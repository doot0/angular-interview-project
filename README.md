...
- Newsfeed stories can have different types. For example, you can have a newsfeed items that say "X added Y as a friend", "P liked Z item", "T uploaded a new photo". The design of the newsfeed items must be consistent (e.g. each have the ability to comment, favourite, though you don't need to implement this), but the actual design must change depending on the type of newsfeed item this is. Hint: newsfeed items have 3 components: subject, action, object (e.g. X[subject] added[action] Y[object]).
...
- You must provide console logs. Additionally, you must modify the console logs so that you get the following behaviour:
